...
result.addEventListener("click", async function (e) {
 if (e.target.classList.contains("fa-vote-yea")) {
  ...
  if(newVotes){
   voteNode.textContent = `${newVotes} ${newVotes === 1 ? "vote" : "votes"}`
   voteNode.dataset.votes = newVotes
  }
 }
})

// refreshBtn.addEventListener("click", async (id, votes) => {
//  console.log("object");

//  modifyData(id, votes = 0)
